                    ...
                    <td colspan="4" style="padding-top:20px;width: 100%;" >
                        ...
                        <tr>
                            <td class="label-td" colspan="2">
                                <label for="duration" class="form-label">Duration per slot (minutes): </label>
                            </td>
                        </tr>
                        <tr>
                            <td class="label-td" colspan="2">
                                <input type="number" name="duration" class="input-text" placeholder="Duration in minutes" value="30" min="15" max="120" step="15" required><br>
                            </td>
                        </tr>
                        </table>
                        </center>
                ...
                $sqlmain= "select schedule.scheduleid,schedule.title,doctor.docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.docid where doctor.docid=$userid ";
                    if($_POST){
                        ...
                        if(isset($_POST["addsession"])){
                            ...
                            $nop = $_POST["nop"];
                            $duration = $_POST["duration"] ?? 30; // Default 30 minutes per slot
                            ...
                            if($scheduledate >= $today){
                                ...
                                if($check_result->num_rows == 0){
                                    ...
                                    $stmt->bind_param("isssi", $userid, $title, $scheduledate, $scheduletime, $nop);
                                    
                                    if($stmt->execute()) {
                                        $schedule_id = $database->insert_id;
                                        
                                        // Generate time slots: only as many as $nop
                                        $start_time = new DateTime($scheduletime);
                                        $slots_sql = "INSERT INTO time_slots (schedule_id, time_slot) VALUES (?, ?)";
                                        $slots_stmt = $database->prepare($slots_sql);
                                        for ($i = 0; $i < $nop; $i++) {
                                            $start_time_str = $start_time->format('H:i:s');
                                            $slots_stmt->bind_param("is", $schedule_id, $start_time_str);
                                            $slots_stmt->execute();
